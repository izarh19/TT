...
export default class Footer extends Component {
  ...
  componentDidMount() {
    fetch("http://localhost:3001/hf/-2", {
      ...
      .then((data) =>
        this.setState({ footerPage: data })
      ) /* put the data that i responsed in footerpage */ /* set state update the footer page in the data it recive */
      .catch((error) =>
    ...
    return (
      <footer>
        <ul className="inside_footer">
          {Array.isArray(footerPage) &&
            footerPage.map((ftr) => (
              <li>
                <a href={ftr.menuURL}>{ftr.menuTitle} </a>
              </li>
            ))}
        </ul>
